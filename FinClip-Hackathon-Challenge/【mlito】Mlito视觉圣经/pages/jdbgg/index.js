...
Page({
  ...
  onShow: function(e){
    if(wx.getStorageSync('vipid')){
      let aa = wx.getStorageSync('vipid');
      this.setData({
          vip_id:aa
      })
      this.vip_bt(aa);
...
vip_zf(){
  ...
  var that = this;
  let bz = that.data.payName+"-"+that.data.name1+"-"+that.data.name2+"-"+that.data.name3;

if(!that.data.name1){return}
  console.log(bz);
  const postData = {
...
vip_bt(e){
  ...
  this.cmsPage("25030"+this.data.vip_id);

  if(id==1){
    this.setData({
      money : "6000",
      nextAction : '{"type":4,"shopId":36264,"money":6000}',
      payName : "小程序开发定金",
      remark :  "小程序开发定金",
      token :  wx.getStorageSync('token')
    })
  ...
  if(id==2){
    this.setData({
      money : "5000",
      nextAction : '{"type":4,"shopId":36264,"money":5000}',
      payName : "网站开发定金",
      remark :  "网站开发定金",
      token :  wx.getStorageSync('token')
    })
  ...
  if(id==3){
    this.setData({
      money : "9999",
      nextAction : '{"type":4,"shopId":36264,"money":9999}',
      payName : "APP开发定金",
      remark :  "APP开发定金",
      token :  wx.getStorageSync('token')
    })
...
console.log("25030"+this.data.vip_id);
  ...
  async cmsPage(a) {
    ...
    if (res.code == 0) {
      ...
      let topicon= sj.sj[0].top.zsj.nr;
      topicon = topicon.split("，");
      console.log("topicontopicontopicon",topicon);
      that.setData({
        topicon:topicon,
        jdata:sj.sj[0],
        ac1a:sj.sj[0].ac1?sj.sj[0].ac1.zh:'',
        ac2a:sj.sj[0].ac2?sj.sj[0].ac2.zh:'',
      ...
      that.data.jdata.ac1?that.lc1(-1,that.data.jdata.ac1.db.link):'';
      that.data.jdata.ac2?that.lc1(-2,that.data.jdata.ac2.db.link):'';
  ...
  onShareAppMessage: function() {    
    return {
      ...
      path: '/pages/coupons/hy?inviter_id=' + wx.getStorageSync('uid')
    }
  },
