        ...
        if (goodsDetailRes.data.basicInfo.pingtuan) {
          ...
          WXAPI.pingtuanSet(goodsId).then(function(pingtuanSetRes) {
            if (pingtuanSetRes.code == 0) {
             ...
             let  ainfo = goodsDetailRes.data.content
             
             ainfo = ainfo.replace(/\<img/g, "<img style='width: 100%;'")
...
// 去拼团
  ...
  onShareAppMessage: function() {
    ...
    if(this.data.pingtuanList){
      var bt = this.data.goodsDetail.basicInfo.characteristic + "大家快来参加我的" + this.data.pingtuanList.setting.numberPersion + "人团";
    }else{
      var bt = this.data.goodsDetail.basicInfo.characteristic;
    }
    console.log(bt);
    console.log(this.data.goodsDetail.basicInfo.pic);
  ...
  //用户点击右上角分享朋友圈
    ...
    console.log(this.data.goodsDetail.basicInfo.pic);
		...
		return {
	      title: bt,
	      query: {
          ctspid: this.data.goodsId,
          ctptid:this.data.ctptid,
          ctid: this.data.ctid
	      },
	      imageUrl: this.data.goodsDetail.basicInfo.pic,
	    }
