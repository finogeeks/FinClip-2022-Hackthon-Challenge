...
Page({
  ...
  onLoad: function (options) {
    this.data.goodsid = options.ctspid
    this.data.ctptid = options.ctptid
    this.data.ctid = options.ctid
  },
  ...
  async onShow() {
    ...
    this.data.name = goodsDetailRes.data.basicInfo.characteristic
...
//     WXAPI.goodsDetail(that.data.goodsid).then(function(goodsDetailRes) {

//       that.data.pic = goodsDetailRes.data.basicInfo.pic
//       that.data.name = goodsDetailRes.data.basicInfo.characteristic
//       that.downLoadGoodsPic()
//     })
...
//   },
  ...
  downLoadQrcode(_imageSize) {
    const _this = this
    console.log(_this.data.goodsid +',' + _this.data.ctptid +',' + _this.data.ctid);
    
    WXAPI.wxaQrcode({
      scene: _this.data.goodsid +',' + _this.data.ctptid +',' + _this.data.ctid,
      page: 'pages/goodsdetails/index',
      is_hyaline: true,
      expireHours: 1
    }).then(res => {
      if (res.code !== 0) {
        ...
        wx.getImageInfo({
          ...
          success: (res) => {
            ...
            ctx.fillText('长按识别小程序码 即可抢购~', _imageSize.windowWidth / 2, _imageSize.imageHeight + 80 + left + 50)
