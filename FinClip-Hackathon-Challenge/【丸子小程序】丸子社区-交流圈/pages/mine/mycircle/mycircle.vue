<template>
	<view class="content">
		...
		<view class="member-info-wrapper">
			<view class="member-wrapper owner">
				...
				<view class="member-list-wrapper">
					...
					<view @tap="onUserClick" class="member-item-wrapper" v-for="(member,memberIndex) in memberList"
		...
		<view class="toplicList" v-if="swithBarIndex==0||swithBarIndex==1">
			<view class="wfw">
				<view class="wf_left">
		...
		<view class="auditList" v-if="swithBarIndex==2">
			<audit-list :posts="posts" @comfirmed='comfirm' v-if="posts"></audit-list>
		</view>
		<view class="sticky-btn flex-center" @tap="gotoPost">
			<text>+</text>
		</view>
		<u-toast ref="uToast" />
	</view>
...
<script>
	...
	export default {
		...
		onReachBottom: function() {
			if (!this.isLastPage && this.swithBarIndex == 0) {
				this.getTopicList({
					circle: this.id,
					page: this.page,
				})
			} else if (!this.isLastPage && this.swithBarIndex == 1) {
				this.getStickyLists({
					circle: this.id,
					page: this.page,
				})

			} else if (!this.data.isLastPage && this.data.swithBarIndex == 2) {
				this.getCircleAudit({
					page: this.page,
				});
			}
		...
		methods: {
			...
			getAuditLists: function(args) {
				API.getAuditList(args).then(res => {
			...
			comfirm: function(e) {
				...
				if (status === '1') {
					...
					API.reViewPostByUser(args).then(res => {
						if (res.status == 200) {
					...
					uni.showModal({
						...
						success: function(res) {
							if (res.cancel) {
								...
								API.deletePostByUser(id).then(res => {
									if (res.status == 200) {
										that.posts.splice(index, 1);
										that.$refs.uToast.show({
											title:'删除成功',
