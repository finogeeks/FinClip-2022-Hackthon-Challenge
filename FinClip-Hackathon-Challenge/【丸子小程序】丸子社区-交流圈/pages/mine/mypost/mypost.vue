		...
		<view class="wfw">
			<view class="wf_left">
				<helang-waterfall v-for="(item,index) in leftList" :key="index" :params="item" tag="left" :index="index"
					@height="onHeight" @click="onClick" @deletePosts="deletePost" :edit="true"></helang-waterfall>
			</view>
			<view class="wf_right">
				<helang-waterfall v-for="(item,index) in rightList" :key="index" :params="item" @height="onHeight"
					@click="onClick" tag="right" :index="index" @deletePosts="deletePost" :edit="true">
				</helang-waterfall>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			deletePost: function(e) {
				...
				API.deletePostByAdmin(e.id).then(res => {
					if (res.status == 200) {
						...
						if (e.tag == 'left') {
							this.leftList.splice(e.index, 1);
						} else {
							this.rightList.splice(e.index, 1);
						}
						uni.hideLoading();
